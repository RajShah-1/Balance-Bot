...
MPULib::MPULib(const byte intPinVal, double fCutVal = 5.0, double alphaVal = 0.03) : interruptPin(intPinVal), fCut(fCutVal), alpha(alphaVal), pitch(0.0), roll(0.0){}
...
// Interrupt Service Response Function
void MPULibISR(void){
    // sensor is declared in the global scope
	
    sensor.dT = (double)(micros()-sensor.timeStamp)/1e6;
	sensor.timeStamp = micros();
    accGyro.getMotion6(&ax, &ay, &az, &gx, &gy, &gz);
    // TauF = 1/(2*PI*fCut);
  	// alphaF = Tau/(Tau+dT);
    sensor.accF = (1-sensor.alphaF)*sensor.acc+sensor.alphaF*sensor.accF;
